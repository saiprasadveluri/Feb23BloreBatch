...
namespace Food_Deliveryapp
{
    public class DataAccessLayer
    {
        ...
        {
            SqlCommand cmd = new SqlCommand($"SELECT Role FROM Users WHERE Uid = {userId}", con);
            string role = (string)cmd.ExecuteScalar();
            Console.WriteLine($"User ID: {userId}, Role: {role}");
            return role == "Admin";
        }

        public bool AddNewRestaurant(RestaurantDTO r, long adminId)
        {
            if (!IsAdmin(adminId))
            {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"INSERT INTO Restaurants (rname, location, ownerid) VALUES ('{r.rname}', '{r.location}', {r.ownerid})";
        ...
        public bool UpdateUser(UserDTO u)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"UPDATE Users SET Name = '{u.Name}', Email = '{u.Email}', Password = '{u.Password}', Address = '{u.Address}', Role = '{u.Role}' WHERE Uid = {u.Uid}";
        ...
        public bool DeleteUser(long userId)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"DELETE FROM Users WHERE Uid = {userId}";
            int rowsAffected = cmd.ExecuteNonQuery();
            return rowsAffected > 0;
        }
        ...
        public bool IsRestaurantOwner(long rid, long uid)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"SELECT COUNT(*) FROM Restaurants WHERE rid = {rid} AND ownerId = {uid}";
        ...
        public List<RestaurantDTO> GetRestaurantsByLocation(string location)
        {
            ...
            string query = $"SELECT rid, rname, location FROM Restaurants WHERE location = '{location}'";
        ...
        public bool AssignOwnerToRestaurant(long rid, long ownerId)
        {
            try
            {
                SqlCommand cmd = new SqlCommand($"UPDATE Restaurants SET OwnerId = {ownerId} WHERE rid = {rid}", con);
                int rowsAffected = cmd.ExecuteNonQuery();
                return rowsAffected > 0;
            }
            catch (Exception ex)
