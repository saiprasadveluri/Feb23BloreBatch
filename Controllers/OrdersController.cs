...
namespace MMVCDemoApp1.Controllers
{
    public class OrdersController : Controller
    {
        ...
        [HttpPost]
        public IActionResult OrderSummary(int RID, string[] mids)
        {
            ...
            foreach (string mid in mids)
            {
                ...
                if (nQty > 0)
                {
                    ...
                    string MenuName = menuItems.Where(m => m.MID == menuId).Select(m => m.MenuName).FirstOrDefault();
                    double Price = menuItems.Where(m => m.MID == menuId).Select(m => m.UnitPrice).FirstOrDefault();
                    old.MenuId = menuId;
