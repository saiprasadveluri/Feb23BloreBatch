...
namespace Store
{
    class Program
    {
        ...
        static void DeleteProduct(Retail.ProductManager manager)
        {
            ...
            Console.Write("Enter Product Type (1.Fashion, 2.Electronics, 3.Kitchen): ");
            ...
            {
                productType = typeof(Retail.Fashion);
            }
            else if (type.Equals("2", StringComparison.OrdinalIgnoreCase))
            {
                productType = typeof(Retail.Electronics);
            }
            else if (type.Equals("3", StringComparison.OrdinalIgnoreCase))
            {
                productType = typeof(Retail.Kitchen);
            }

            if (productType != null)
            {
                manager.DeleteProduct(pid, productType);
                Console.WriteLine("Product deleted successfully.");
            }
            else
            {
                Console.WriteLine("Invalid product type entered. Please try again.");
            }
        }
    }
}
