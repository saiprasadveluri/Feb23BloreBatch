...
namespace FoodDelivery
{
    ...
    {
        const string ConString = "Data Source=.;Initial Catalog=FOOD;Integrated Security=SSPI";
        ...
        public UserDTO Login(string Email, string Password)
        {
            ...
            cmd.Connection = con;
            cmd.CommandText = $"SELECT * FROM USERSS where Email='{Email}' AND Password='{Password}'";
        ...
        public bool AddNewRestaurant(RestaurantDTO restaurant)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"INSERT INTO Restaurant (RName,Location,OwnerId) VALUES('{restaurant.Name}','{restaurant.Location}',{restaurant.OwnerId}";
        ...
        public bool AddNewUser(UserDTO user)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"INSERT INTO USERS (DisplayName,Email,Password,RoleName,Location) VALUES('{user.Name}','{user.Email}','{user.Password}','{user.RoleName}','{user.Location}')";
            int RowsEffected = cmd.ExecuteNonQuery();
            if (RowsEffected > 0)
            {
                return true;
        ...
        public string GetUserRole(long UserId)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"Select RoleName from USERSS where UserId={UserId}";
        ...
        public List<UserDTO> GetRestaurantOwnersList()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"SELECT * FROM USERSS where RoleName='{UserTypeEnum.OWNER}'";
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
