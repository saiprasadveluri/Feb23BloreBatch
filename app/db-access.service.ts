...
export class DbAccessService {
  ...
  AddNewProject(inpProj: ProjectInfo): Observable<any> {
    return this.http.get<ProjectInfo[]>(`${this.baseUrl}/ProjectInfo`).pipe(
      map(projects => {
        const maxId = projects.reduce((max, p) => Math.max(max, p.id ? parseInt(p.id, 10) : 0), 0);
  ...
  GetProjectTasks(projId: string): Observable<Task[]> {
    return this.http.get<Task[]>(`${this.baseUrl}/projects/${projId}/tasks`);
  }
}
