...
export class ProjectDashboardComponent implements OnInit {
  ...
  assignMember() {
    ...
    if (selectedUser) {
      const newMember: ProjectMember = {
        ...
        role: selectedUser.role as 'QA' | 'Developer'
      };
  ...
  removeMember(newMemberName: string) {
    this.dbService.removeMemberFromProject(this.projectId, newMemberName).subscribe(
      () => {
        this.loadMembers(); 
      },
      (error) => {
  ...
  createTask() {
    ...
    if (assignedMember) {
      ...
      console.error('No valid member selected for assignment.');
    }
  }
