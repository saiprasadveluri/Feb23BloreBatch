...
namespace FoodDelApp
{
    public class DataAccessLayer
    {
        ...
        public bool DeleteRestaurant(long RID)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"DELETE Restaurant WHERE RID={RID}";
            int RowsEffected = cmd.ExecuteNonQuery();
            if (RowsEffected > 0)
            {
                return true;
        ...
        public List<RestaurantDTO> ListRestaurantsByLocation(string UserLocacation)
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"SELECT * FROM Restaurant where Location='{UserLocacation}'";
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
        ...
        public List<RestaurantDTO> ListRestaurants()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = @$"SELECT R.RID,R.Name,R.Location,U.DisplayName FROM Restaurant R
        ...
        public bool OrderMenuItem(long OrderID, long MenuId, int QTY)
        {
            if (trans != null)
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = $"INSERT INTO OrderListItems (ORDERID,MenuId,Qty) VALUES({OrderID},{MenuId},{QTY})";
        ...
        public List<OrderDTO> GetOrdersByUser(long UserId)
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = @$"SELECT O.OderId,R.NAME,U.DISPLAYNAME,O.OrderDate,O.Status
                                FROM Orders O 
        ...
        public List<OrderDTO> GetOrdersByRestaurant(long RID)
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = @$"SELECT O.OrderId,R.Name,U.DisplayName,O.OrderItem,O.Status
                                FROM Orders O 
                                JOIN Restaurant R ON O.RID=R.RID 
                                JOIN UserInfo U ON O.OrderBy=U.USERID
                                where O.rid={RID}";
